...
#       scmd 0x08 # to detach the base
#       scmd 0x08 && sleep 0.1 && scmd 0x09 # to detach the base quickly
i2cbus=$(realpath /sys/bus/i2c/devices/i2c-MSHW0030:00 | cut -d"/" -f7 | cut -d"-" -f2)
chip_address=0x28
scmd(){
    sudo i2cset -f -y $i2cbus $chip_address 0x05 0x00 0x3f 0x03 0x24 0x06 0x00 0x13 0x00 0x24 "$1" \
